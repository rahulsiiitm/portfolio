...
export default function ChatWidget() {
  ...
  useEffect(() => {
    ...
    const pingTimer = window.setTimeout(() => void ping(), 1200);
    return () => {
      active = false;
  ...
  return (
    <>
      {isOpen && <ChatWindow serverState={serverState} onClose={() => { sfx.close(); setIsOpen(false); }} />}

      <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 flex items-center justify-end pointer-events-none">
        {!isOpen && (
          <div className="zero-chat-hint hidden lg:flex items-center gap-2 mr-4 pointer-events-none">
        ...
        <div className="relative pointer-events-auto">
          ...
          <button
            className="w-[60px] h-[60px] sm:w-16 sm:h-16 rounded-full bg-black border border-racing-red shadow-[0_10px_28px_rgba(0,0,0,0.35),0_0_14px_rgba(255,0,0,0.35)] flex items-center justify-center relative group z-10 overflow-hidden transition-[transform,box-shadow] duration-200 hover:scale-[1.04] hover:shadow-[0_0_20px_rgba(255,0,0,0.55)] active:scale-95"
            onClick={toggle}
